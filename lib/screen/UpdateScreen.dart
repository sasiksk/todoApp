...
class UpdateToDo extends StatefulWidget {
  final ToDoList existingToDo;

  const UpdateToDo({Key? key, required this.existingToDo}) : super(key: key);
...
class _UpdateToDoState extends State<UpdateToDo> {
  ...
  void initState() {
    ...
    DatabaseHelper.getDatabase().then((db) {
      _loadCategories();
      _initializeValues(); // Move this call here
    });
  ...
  void _initializeValues() {
    _todotext.text = widget.existingToDo.tododetails;
    if (widget.existingToDo.date != null) {
      _selectedDate = widget.existingToDo.date;
      _tododate.text = DateFormat.yMMMd().format(_selectedDate!);
    }
    if (widget.existingToDo.time != null) {
      _selectedTime = TimeOfDay.fromDateTime(widget.existingToDo.time!);
      _todotime.text = _selectedTime!.format(context);
    }
    _isChecked = widget.existingToDo.alarm ?? false;
  }
    ...
    try {
      ...
      await db.update(
        'todolist',
        {
          ...
          'alarm': _isChecked ? 1 : 0,
          'status': 'Not Completed',
        },
        where: 'tid = ?',
        whereArgs: [widget.existingToDo.tid],
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: const Text(
                'An error occurred while updating the todo. Please try again.'),
            actions: <Widget>[
              TextButton(
                onPressed: () {
                  Navigator.of(context).pop();
